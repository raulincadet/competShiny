...
df<-df%>%
  select(`Country Code`,`Country Name`,Region,IncomeGroup,Cases,Deaths,Population,PIBcapita,Tourists,HDI)
...
```{r}
renderDygraph(
  gra_xts(data=covid_ts,country=input$countries,variable=c('total_cases',"people_vaccinated"))%>%
    dyRangeSelector()
)
```
...
```{r}
renderDygraph(
  gra_xts(data=covid_ts,country=input$countries,variable='new_vaccinations')%>%
    dyRangeSelector()
)
