...
#hdi<-covid_ts%>% # Human Development Index (HDI)
 # group_by(location)%>%
  #summarise(tail(human_development_index,1))%>%
  #rename("Country Name"="location","HDI"="tail(human_development_index, 1)")

##############3
dfts<-covid_ts%>% # Human Development Index (HDI)
  group_by(location)%>%
  summarise(HDI=tail(human_development_index,1),New_cases=tail(new_cases,1),
            New_deaths=tail(new_deaths,1))%>%
...
#colnames(hdi)
############## Merging tibbles #####################
df<-inner_join(inner_join(inner_join(inner_join(inner_join(covid,pib_capita19,by="Country Name"),
...
df<-df%>%
  select(`Country Code`,`Country Name`,Region,IncomeGroup,Cases,Deaths,Population,PIBcapita,Tourists,HDI,New_cases,New_deaths)
...
```{r}
renderDygraph(
  gra_xts(data=covid_ts,country=input$countries,variable='new_cases',main=paste("Total cases in",input$countries))%>%
    dyRangeSelector()
...
```{r}
renderDygraph(
  gra_xts(data=covid_ts,country=input$countries,variable=c('total_cases',"people_vaccinated"),main=paste("Total cases, compared to people vaccinated in",input$countries))%>%
...
dygraph(totnewcases,main = "Total of cases worldwide")%>%
  dyRangeSelector()
```
...
totpeoplevacc<-xts(tot$tot_people_vaccinated,order.by = as.Date.character(tot$date))
  ...
  dygraph(cbind(Cases=totcases,`People vaccinated`=totpeoplevacc),main = "Total cases worldwide, compared to people vaccinated")%>%
  dyRangeSelector()%>%
    dyOptions(fillGraph = T)
